...
| **VM count** | 4 (Root CA, DC, Issuing CA, Web Server) | 3 (CA Server, Web Server, Client) |
...
and an IIS web server — all issuing and validating ML-DSA certificates with ML-KEM TLS key
exchange.
...
webserver01     IIS Web Server           ML-DSA-65 TLS cert + x25519_mlkem768
```
...
| **Cost** | ~$380/month running 24/7; ~$30/month when deallocated |
...
| **RAM** | 16+ GB recommended (4 VMs × 4 GB each) |
| **Disk** | ~200 GB free |
...
| Browser compatibility (Windows path) | Microsoft Edge (CNG-based) validates ML-DSA chains. Chrome/Firefox support unconfirmed as of August 2026. Use Edge for testing. |
